...
const ProfessorRequestStatusChart: React.FC = () => {
  const total = data.reduce((sum, entry) => sum + entry.value, 0);

  return (
    <ChartContainer title="Status das Minhas Requisições">
      {data.length > 0 ? (
        <>
          <div className="relative w-full h-64">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                ...
                <span className="text-sm text-light-text">Total</span>
            </div>
          </div>
...
export default ProfessorRequestStatusChart;