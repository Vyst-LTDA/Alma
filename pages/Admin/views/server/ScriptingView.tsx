...
const ScriptingView: React.FC = () => {
    ...
    const handleSaveScript = async () => {
        ...
        try {
            ...
            if (isNew) {
                ...
                if (originalScript && originalScript.hook !== editorState.hook && originalScript.hookId) {
                    const updateHookCmd: UpdateHookCommand = {
                        id: originalScript.hookId,
                        scriptId: editorState.id,
                        hookName: editorState.hook,
                    };
                    await updateHook(updateHookCmd);
                }
            }
            alert('Script salvo com sucesso!');
            setActiveScriptId(null); // Deselect to force re-fetch and avoid issues with temp ID
            await fetchData();
        } catch (error: any) {
    ...
    const handleDeleteScript = async () => {
        ...
        if (isNew) {
            setScripts(prev => prev.filter(s => s.id !== activeScriptId));
            setActiveScriptId(null);
            return;
        }
                        ...
                        <>
                            <div className="flex justify-between items-start p-4 border-b border-gray-200 gap-4">
                                ...
                                <div className="flex items-start gap-2 flex-shrink-0">
                                     <button onClick={() => setIsTestModalOpen(true)} className="px-3 py-1.5 text-xs font-semibold bg-blue-100 text-blue-700 rounded-md hover:bg-blue-200">
                                        Testar
                                    </button>
